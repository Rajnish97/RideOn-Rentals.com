jQuery(function() {
    ...
    if (dig_login_wp_elem.length) {
        if (dig_mdet.login_mobile_accept > 0) {
            ...
            if (dig_login_wp_elem.find(".loginuname").length === 0) {
                ...
                dig_login_wp_elem.find("input[type='text']").first().attr({
                    id: "username",
                    nan: 1
                })
            } else {
    ...
    function loginuser(response) {
        ...
        jQuery.ajax({
            ...
            success: function(res) {
                ...
                loader.hide();
                if (res == "1") {
                    if (ihc_loginform == 10) document.location.href = "/";
                    else {
                        showDigLoginSuccessMessage();
    ...
    if (c.length && dig_mdet.mobile_accept > 0) {
        ...
        jQuery('<input type="hidden" id="dig_ihc_ea_code" name="code"/><input type="hidden" id="dig_ihc_ea_csrf" name="csrf"/><div id="dig_ihc_mobotp" class="iump-form-line-register iump-form-text" style="display:none;">' + '<input value="" id="digits_otp_ihc" name="digit_otp" placeholder="' + dig_mdet.OTP + '" type="text" style="padding-left:10px !important;">').insertBefore(f.find("input[type='submit']").closest(".iump-submit-form"))
    }
    ...
    if (uc.length) {
        ...
        if (createAccount.length) {
            createAccount.find("#username").attr({
                "f-mob": 1,
                reg: 1,
                "data-dig-mob": 1
            });
    ...
    if (um_login.length) {
        if (um_login.find(".um-field-mobile_number").length) {
            ...
            um_username_field.find("input").attr({
                id: "username",
                nan: 1
            });
    ...
    if (um_forgot.length) {
        ...
        if (um_forgot_uname_field.length) {
            ...
            um_forgot_uname_field.find("input").attr({
                id: "username",
                nan: 1
            });
    ...
    function update_username_field($this) {
        ...
        usernameid.on("keyup change focusin", function(e) {
            ...
            if (dig_main) {
                ...
                if (isNumeric(con) && con.length > 0 && isNumeric(ccdval) && ccdval.length > 0) jQuery("#" + dig_main).val(ccdval + "" + con);
    ...
    jQuery(".dig_wc_registersecondcountrycode").on("keyup change focusin", function(e) {
        ...
        curRegMail.css({
            "padding-left": mobilePaddingLeft(dwccr.outerWidth() + ew / 2 + "px")
        }, "fast", function() {})
    });
    ...
    jQuery(".dig_wc_registercountrycode").on("keyup change focusin", function(e) {
        ...
        curRegMail.css({
            "padding-left": mobilePaddingLeft(rccBox.outerWidth() + ew / 2 + "px")
        }, "fast", function() {});
        updateMailSecondLabel(curRegMail)
    });
    user_login.on("keyup change focusin", function(e) {
        if (isNumeric(jQuery(this).val())) {
            jQuery(".forgotcountrycodecontainer").css({
                display: "inline-block"
            });
    ...
    jQuery(".forgotcountrycode").on("keyup change focusin", function(e) {
        ...
        user_login.css({
            "padding-left": mobilePaddingLeft(jQuery(".forgotcountrycode").outerWidth(true) + ew / 2 + "px")
        }, "fast", function() {})
    });
    ...
    function updateMailSecondLabel(reg_email) {
        ...
        if (isNumeric(con) && inftype != 1 || dig_mdet.mail_accept == 2) {
            ...
            secondmailormobile.css({
                "padding-left": "0.75em"
            })
        } else if (!isNumeric(con) && inftype != 2 && dig_mdet.mobile_accept != 2) {
            inftype = 2;
            mailSecondLabel.html(dig_mdet.MobileNumber);
            digSecondCountryCode.css({
                display: "inline-block"
            });
            secondmailormobile.css({
                "padding-left": mobilePaddingLeft(digSecondCountryCode.find(".dig_wc_registersecondcountrycode").outerWidth() + ew / 2 + "px")
            })
        }
    ...
    register.find(".woocommerce-Button, button[name='register']").each(function() {
        if (jQuery(this).attr("name") == "register") {
            if (!jQuery(this).hasClass("otp_reg_dig_wc")) {
                if (jQuery(".otp_reg_dig_wc").length) jQuery(this).val(dig_mdet.RegisterWithPassword).text(dig_mdet.RegisterWithPassword).addClass("wc_reg_pass_btn")
            }
        }
    });
    register.off();
    ...
    function hideloginpageitems() {
        ...
        if (cuForm.hasClass("wp_login")) {
            ...
            wp_login.children().each(function() {
                if (!jQuery(this).hasClass("dig_otp_block") && jQuery(this).find(".digcon").length === 0 && jQuery(this).find(".dig_bb_wp_otp_field").length === 0) {
                    jQuery(this).hide()
                }
            });
        ...
        cuForm.find(".form-row").find("input[type='password']").each(function(index) {
            ...
            if (index != 1 && mrow.attr("otp") != 1) mrow.remove();
            else if (index == 1) {
    ...
    if (jQuery("#dig_wc_check_page").length && dig_mdet.mob_verify_checkout == 1 && dig_mdet.mobile_accept > 0) {
        ...
        createAccount.append("" + "<div class='dig_billing_wc_dv'>" + digchbtn + '</div><div  class="dig_resendotp dig_wcbil_bill_resend" id="dig_man_resend_otp_btn" dis="1">' + dig_mdet.resendOTP + " <span>(00:<span>30</span>)</span></div>" + "<a id='dig_billing_validate_button' style='display:none;'></a><a id='dig_billing_signupwithpassword' style='display:none;'></a><br /> ");
        var tasc = 0;
    ...
    if (ihcloginform.length && dig_mdet.login_mobile_accept > 0) {
        ...
        usernameihc.wrap('<div class="digcon"></div>').before('<div class="dig_ihc_countrycodecontainer dig_ihc_logincountrycodecontainer" style="display: none;">' + '<input type="text" name="countrycode" class="input-text countrycode dig_ihc_logincountrycode" ' + 'value="' + ccd + '" maxlength="6" size="3" placeholder="' + ccd + '" style="position: absolute;top:0;"/></div>');
        usernameihc.on("keyup change focusin", function(e) {
            if (isNumeric(jQuery(this).val())) {
                jQuery(".dig_ihc_logincountrycodecontainer").css({
                    display: "inline-block"
                });
        ...
        jQuery('<div class="impu-form-line-fr impu-dig-otp" id="impu-dig-otp" style="display: none;">' + '<input value="" id="digits_otp_ihc" name="digit_otp" placeholder="' + dig_mdet.OTP + '" type="text" style="padding-left:10px !important;">').insertBefore("#ihc_login_form .impu-form-submit");
            ...
            if (isNumeric(usernameihc.val())) {
                ...
                if (remotp == 0) {
                    ...
                    ihcloginform.find("div").each(function(index) {
                        if (index > 1) {
                            if (!jQuery(this).hasClass("impu-form-submit") && !jQuery(this).hasClass("dig_ihc_logincountrycodecontainer") && !jQuery(this).hasClass("impu-dig-otp")) jQuery(this).hide()
                        }
                    })
                }
            }
        ...
        if (dig_mdet.login_otp_accept > 0) {
            ihcloginform.find(".impu-form-submit").append("<div id='digorimp'> " + dig_mdet.or + "<br /><br /></div>" + "<input type='submit' id='impu_log_submit' value='" + dig_mdet.loginwithotp + "' />" + "<div class='dig_resendotp dig_impu_login_resend' id='dig_man_resend_otp_btn' dis='1'>" + dig_mdet.resendOTP + " <span>(00:<span>30</span>)</span></div></div>")
        }
    }
    var verifyimpuotp = 0;
    var ihc_loginform = 0;
    ...
    if (ihforgaction.val() == "reset_pass" && dig_mdet.forgot_pass > 0) {
        ...
        jQuery("<div class='dig_resendotp dig_impu_forg_resend' id='dig_man_resend_otp_btn' dis='1'>" + dig_mdet.resendOTP + " <span>(00:<span>30</span>)</span></div>").insertAfter(ihcforgsub);
        ...
        var ccd = dig_mdet.uccode;
        ihcForgotUsername.wrap('<div class="digcon"></div>').before('<div class="dig_ihc_forgot_countrycodecontainer dig_ihc_forgot_logincountrycodecontainer" style="display: none;">' + '<input type="text" name="countrycode" class="input-text countrycode dig_ihc_forgotcountrycode" ' + 'value="' + ccd + '" maxlength="6" size="3" placeholder="' + ccd + '" style="position: absolute;top:0;"/></div>');
        ihcForgotUsername.attr("placeholder", dig_mdet.UsernameMobileno);
        jQuery('<div class="impu-form-line-fr" id="digits_password_ihc_cont" style="display: none;"><input value="" id="digits_password_ihc" name="digits_password" placeholder="' + dig_mdet.Password + '" type="password" style="padding-left:10px !important;"></div>' + '<div class="impu-form-line-fr" id="digits_cpassword_ihc_cont" style="display: none;"><input value="" id="digits_cpassword_ihc" name="digits_cpassword" placeholder="' + dig_mdet.ConfirmPassword + '" type="password" style="padding-left:10px !important;"></div>').insertAfter(ihcForgotUsername.closest(".impu-form-line-fr"));
        jQuery('<div class="impu-form-line-fr impu-dig-otp" id="impu-dig-otp" style="display: none;"><input value="" id="digits_otp_forg_ihc" name="dig_otp" placeholder="' + dig_mdet.OTP + '" type="text" style="padding-left:10px !important;"></div>').insertAfter(ihcForgotUsername.closest(".impu-form-line-fr"));
        jQuery('<input type="hidden" name="code" id="digits_impu_code"/><input type="hidden" name="csrf" id="digits_impu_csrf"/>').insertAfter(ihcForgotUsername.closest(".impu-form-line-fr"));
        ihcForgotUsername.on("keyup change focusin", function(e) {
            if (isNumeric(jQuery(this).val())) {
                jQuery(".dig_ihc_forgot_countrycodecontainer").css({
                    display: "inline-block"
                });
    ...
    function validateCustomFields(form) {
        ...
        form.find("input,textarea,select").each(function() {
            if (jQuery(this).attr("required") && jQuery(this).is(":visible") || jQuery(this).attr("data-req")) {
                var $this = jQuery(this);
                ...
                if ($this.attr("date")) {
                    ...
                    if (value == null || value.length == 0 || value == -1 && $this.is("select")) {
                        error = true;
                        if ($this.is("select")) $this.next().addClass("dig_input_error");
                        $this.trigger("focus")
                    }
                }
            }
